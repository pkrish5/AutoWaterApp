...
import '../widgets/weekly_tasks_widget.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  try {
    ...
    final tasks = await api.getWeeklyTasks(auth.userId!);
    if (mounted) {
      setState(() {
        _weeklyTasks = tasks;
      });
    ...
    debugPrint('Failed to load weekly tasks: $e');
  }
}
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text('Stay', style: GoogleFonts.quicksand(color: AppTheme.leafGreen, fontWeight: FontWeight.w600)),
          ),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(children: [
                      ...
                      if (_weeklyTasks != null) ...[
                        WeeklyTasksCard(tasks: _weeklyTasks!),
                        const SizedBox(height: 20),
                      ],
                      _buildLocationCard(),
                      const SizedBox(height: 20),
...
Widget _buildHeader() => Row(children: [
  ...
  if (_weeklyTasks != null || (_user?.totalPoints ?? 0) > 0)
    Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [AppTheme.sunYellow, AppTheme.sunYellow.withValues(alpha: 0.85)],
        ),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            _formatPoints(_user?.totalPoints ?? 0),
...
String _formatPoints(int points) {
  ...
  Widget _buildProfileCard(AuthService auth) => Container(
    ...
    child: Column(children: [
      ...
      if (_isEditingUsername)
        ...
        Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(
              _user?.username ?? auth.userEmail?.split('@').first ?? 'User',
              style: GoogleFonts.comfortaa(
                fontSize: 22,
                fontWeight: FontWeight.bold,
      ...
      const SizedBox(height: 8),
      // Points display
      if (_user?.totalPoints != null && _user!.totalPoints! > 0)
        Container(
          margin: const EdgeInsets.only(bottom: 8),
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [AppTheme.sunYellow, AppTheme.sunYellow.withValues(alpha: 0.7)],
            ),
            borderRadius: BorderRadius.circular(16),
          ),
          child: Row(
  ...
  Widget _buildLocationCard() => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Icon(Icons.location_on, color: AppTheme.leafGreen),
      ...
      if (_user?.location?.timezone != null)
        Text('Timezone: ${_user!.location!.timezone}', style: GoogleFonts.quicksand(fontSize: 12, color: AppTheme.soilBrown.withValues(alpha: 0.5))),
    ]),
  );
  ...
  Widget _buildSettingsCard() => Container(
    ...
    child: Column(children: [
      ...
      _SettingsTile(
        ...
        title: 'Notifications',
        trailing: Switch(
          value: _user?.settings?.pushNotificationsEnabled ?? true,
          onChanged: (_) {},
      ...
      _SettingsTile(
        ...
        trailing: Switch(
          value: _user?.isPublicProfile ?? false,
          onChanged: (_) {},
          activeThumbColor: AppTheme.leafGreen,
        ),
      ),
...
class _SettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      leading: Icon(icon, color: iconColor ?? AppTheme.leafGreen),
      title: Text(title, style: GoogleFonts.quicksand(fontWeight: FontWeight.w600, color: AppTheme.soilBrown)),
      trailing: trailing ?? const Icon(Icons.chevron_right, color: AppTheme.soilBrown),
      onTap: onTap,
    );
  }
}
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              final r = _results[i];
              return ListTile(
                leading: const Icon(Icons.location_on, color: AppTheme.leafGreen),
                title: Text(r.displayName, maxLines: 2, overflow: TextOverflow.ellipsis, style: GoogleFonts.quicksand()),
